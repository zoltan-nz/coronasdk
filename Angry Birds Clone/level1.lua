...
function new()
	...
	physics.setScale(50)
	physics.start()
	shootSound = audio.loadStream( "wee.wav" )
...
local floor = display.newRect( -512,80, 8000, 2 )
	physics.addBody(floor, "static", {density = 1.0, friction = 0.9, bounce = 0.2,
	isSensor = false})
...
local grass2 = display.newImage("ground.png")
	grass2.y = - 50
	grass2.x = grass.x + grass.width
	gameGroup:insert(grass2)
...
physics.addBody(character, "dynamic", {density = 2.0, friction = 0.3, bounce = 0.5,
	radius =50})
...
function makePlatform(x,y)
	...
	physics.addBody(monster, "dynamic", {density = 1.0, friction = 0.3, 
		bounce = 0.2, isSensor = false})
	physics.addBody(left, "dynamic", {density = 1.0, friction = 0.3, 
		bounce = 0.2, isSensor = false})
	physics.addBody(right, "dynamic", {density = 1.0, friction = 0.3, 
		bounce = 0.2, isSensor = false})
	physics.addBody(top, "dynamic", {density = 1.0, friction = 0.3, 
		bounce = 0.2, isSensor = false})
